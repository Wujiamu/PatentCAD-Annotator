...
| 特性 | 2007 版 | 2013 版 |
|------|---------|---------|
| 标注方式 | Leader + MText（分离实体） | MLeader（F 方案三点顶点链） |
...
- 标注用 **MLeader（F 方案）**：单个多重引线实体自持 MText，顶点链为 `附着点 → 拐点… → 文字点`（文字点始终是最后一个顶点），并禁用全部自动几何（dogleg/landing/extend），绘制路径与用户点击点完全一致，无鱼钩、无额外附着点。
...
- `ExtendLeaderToText` 为 2014+ SDK 属性（2013 无此成员），代码中以反射访问。
