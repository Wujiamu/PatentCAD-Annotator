...
> **状态：已完成（1.0.0）** — 含样条曲线、无箭头、箭头尺寸、全选、字典比对等增强功能。
>
...
- 面板标注请求按当前图纸隔离并去重；若 CAD 正在执行其他命令，会在空闲后自动补发一次 `PATMARK`，不会叠加失效命令。
...
将 `<dwg文件名>.dict.json` 放在 DWG 同目录。由 `PatentMarker-2007-deploy/vba/` 下的 VBA 宏从 Word 说明书提取生成。自 1.0.0 起该文件带"隐藏+系统"属性，资源管理器默认不可见（文件夹选项 → 显示隐藏的文件，并取消"隐藏受保护的操作系统文件"可查看）。
