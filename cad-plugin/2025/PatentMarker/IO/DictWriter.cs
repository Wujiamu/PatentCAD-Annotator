...
namespace PatentMarker.IO
{
    ...
    /// <summary>
    /// dict.json 序列化与写回（v4.0 新增，CAD 端手动编辑的落盘通道）。
    ...
    public static class DictWriter
    {
        private static readonly JsonSerializerOptions JsonOpts = new()
        {
            ...
            // VBA 输出原始 UTF-8 中文，此处关闭默认的 \uXXXX 转义
        ...
        /// 行尾统一为 \r\n（与 VBA 字符串拼接输出一致）。
        /// 注意：System.Text.Json 在 .NET 9+ 运行时（Windows）输出 CRLF，
        /// .NET 8 输出 LF，此处用正则统一，避免运行时差异产生 \r\r\n。
        /// </summary>
        ...
        /// 避免写入中断产生半截 JSON；随后清空该文件的 FileSystem 属性缓存不适用，
        /// 由调用方（DictLoader.NotifySelfWrite）同步缓存。
        /// </summary>
