...
namespace PatentMarker.Commands
{
    public class PatMLeaderVerifyCommand
    {
        ...
        {
            /// <summary>采样折线段（相邻采样点连线，覆盖 Line/Spline/Polyline）。</summary>
            public List<Point3d[]> Segments = new List<Point3d[]>();

            /// <summary>曲线端点集合（用于 C1 附着点与 C4 单附着判定）。</summary>
            public List<Point3d> CurveEndpoints = new List<Point3d>();

            /// <summary>参与绘制的曲线类型名（用于 C6 直线模式判定）。</summary>
            public List<string> CurveTypes = new List<string>();

            /// <summary>非曲线、非箭头 Solid、非 MText、非空箭头块的实体类型。</summary>
        ...
        public void Run()
        {
            ...
            if (reportDir == null || reportDir.Trim().Length == 0)
        ...
        private bool VerifyOne(Transaction tr, MLeader ml, StringBuilder report)
        {
            ...
            report.AppendLine(string.Format(CultureInfo.InvariantCulture,
                "  entity: textLocation={0} height={1} leaderType={2} arrowSize={3} dogleg={4} landing={5}",
                P(textLoc), textHeight.ToString("F2", CultureInfo.InvariantCulture),
                ml.LeaderLineType, arrowSize.ToString("F2", CultureInfo.InvariantCulture),
                ml.EnableDogleg, ml.EnableLanding));
            ...
            report.AppendLine("  drawn segments=" + ex.Segments.Count
                + " curveTypes=[" + string.Join(",", ex.CurveTypes.ToArray()) + "]"
            ...
            double attachTol = hasArrow ? arrowSize + 0.5 : 0.5;
            Report(report, ref ok, bestAttach <= attachTol,
                "C1 attach-on-path-start", bestAttach.ToString("F3")
                + " (tol=" + attachTol.ToString("F1") + ")");

            // C2 每个用户拐点都在绘制路径上（样条按采样点校验，容差放宽）
            double doglegTol = isSplined ? 1.0 : 0.5;
            for (int i = 0; i < doglegs.Count; i++)
            {
                double d = MinDistanceToPath(ex, doglegs[i]);
                Report(report, ref ok, d <= doglegTol,
                    "C2 dogleg" + i + "-on-path", d.ToString("F3")
                    + " (tol=" + doglegTol.ToString("F1") + ")");
            }
            ...
            // C4 多附着点检测：距文字 1×字高内曲线端点数 ≤ 1
            // （只统计曲线端点，不统计采样点，避免样条采样误报）
            int near = 0;
            foreach (Point3d endpoint in ex.CurveEndpoints)
            {
                if (endpoint.DistanceTo(textLoc) < textHeight) near++;
            }
            ...
            // C6 直线模式 → 所有曲线均为直线几何载体（Line 或 Polyline，
            // Explode 产物因 AutoCAD 版本而异：新版本输出单条 Polyline）
            if (!isSplined)
            {
                bool allLines = ex.OtherTypes.Count == 0;
                foreach (string curveType in ex.CurveTypes)
                {
                    if (curveType != "Line" && curveType != "Polyline") allLines = false;
                }
                Report(report, ref ok, allLines, "C6 straight-all-lines",
                    "curves=[" + string.Join(",", ex.CurveTypes.ToArray()) + "]");
            }
        ...
        private static Exploded ExplodeOne(MLeader ml)
        {
            try
            {
                ...
                foreach (DBObject o in objs)
                {
                    ...
                    BlockReference br = o as BlockReference;
                    if (br != null) { ex.BlockRefCount++; continue; } // 空箭头块
        ...
        /// <summary>统一解析 Line/Spline/Polyline 等曲线：记录端点 + 弧长采样段。</summary>
        private static void AddCurve(Exploded ex, Curve curve)
        {
            try
            {
                ex.CurveTypes.Add(curve.GetType().Name);
                ...
                if (length < 1e-9)
                {
                    ex.Segments.Add(new Point3d[] { start, end });
                    return;
                }

                // 约 2 个图形单位一个采样点，上限 500 段
                int samples = (int)Math.Ceiling(length / 2.0);
                if (samples < 1) samples = 1;
                if (samples > 500) samples = 500;

                Point3d previous = curve.GetPointAtDist(0.0);
                for (int i = 1; i <= samples; i++)
                {
                    Point3d current = curve.GetPointAtDist(
                        length * i / samples);
                    ex.Segments.Add(new Point3d[] { previous, current });
                    previous = current;
                }
            }
            catch
        ...
        private static double MinDistanceToEndpoint(Exploded ex, Point3d pt)
        {
            ...
            foreach (Point3d endpoint in ex.CurveEndpoints)
                best = Math.Min(best, endpoint.DistanceTo(pt));
            return best;
        }
