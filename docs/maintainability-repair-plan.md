...
## 15. 2026-09-05 本机收尾与剩余边界

- PATCHECK 状态已按 AutoCAD `Document` 对象隔离，并在文档关闭时释放；编号查询与字典身份保持大小写不敏感。2025 回归套件实际通过 `117/117`，2010/2013/2015/2007 模拟契约各 `33/33`。
- Word 导出已在本机 Word COM 通过动态 DWG 映射、歧义拒绝、Save As/普通保存失败处理和原子替换验证；8 份受跟踪脱敏语料由真实 VBA 重新生成，SHA256 与基线一致。五套 VBA 文件仍由根 `vba/` 同源并通过同步检查。
- 五版 DLL 重新编译成功，2013/2015 ILRepack 单文件检查通过；发行暂存只包含发布资产，部署 DLL 与暂存内容逐版哈希一致。2025 安装脚本已覆盖 R25.0/R25.1/R26.0 并存配置。
- 2010/2013/2015 安装脚本已改为枚举全部支持范围内的注册表配置；2010 的 HKLM 和 Support/acad.lsp 兜底也逐配置尝试。各版 VBScript 已完成本机 `cscript` 解析验证，`tools/check-installer-contract.ps1` 已纳入 `build.ps1 -Static`，未在对应旧版 AutoCAD 主机执行安装回归。
- AutoCAD 2026 Core Console 使用 2025 部署 DLL 批处理执行 `PATBRACE` 并 `LIST` 读取实体，顶点范围符合 `x=0..50、y=0..100` 的修复预期；随后以 `PATBRACEEDIT` 尺寸模式将 100×50 改为 120×60，更新命令和保存结果均通过。上述证据证明命令级几何与原位重建，但仍不替代 GUI 目检。

仍需外部宿主或人工执行的项目保持不变：AutoCAD 2007/2010/2013/2015 真宿主加载与交互、2025/2026 面板级鼠标/对话框回归，以及对旧大括号执行 `PATBRACEEDIT` 的现场确认。2007 只有一个 R17.0 版本根，但安装器已合并 HKCU/HKLM 中的全部产品配置；五版安装/卸载并存注册表逻辑已完成本机脚本解析与静态契约验证，2010 卸载还会清理生成的 acad.lsp 片段；真实旧版主机安装回归仍待对应环境。
