...
- Leader/MText 关联后附着点复核（2026-08-11）：2007 实测显示，仅在 `Leader.Annotation` 关联前设置 MText 象限时，旧版 AutoCAD 可能重新规范化文字附着，导致上方场景仍落到底部。五个版本现在在关联完成后再次写入附着点和文字位置，并记录关联后的实际值；象限判断使用靠近文字的最后一个引线拐点，保持 Leader 的实际关联规则。
