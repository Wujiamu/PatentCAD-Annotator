...
**2007 大括号反向排查结论（2026-09-03）**：同事反馈 2007 版 `PATBRACE` 大括号方向反向（中部尖点与宽度点击侧相反，与修复前"单侧双鼓包"同一现象）。全面排查后确认**当前源码与五套部署包均无此 bug**，证据如下：

- 几何为共享单源 `Shared/Commands/PatBraceGeometry.cs`（PPT 轮廓：尖点在第三点所指侧、外侧肩与直茎在相反侧）；2007 契约模拟测试 `28/28` 通过，含 `WidthPointControlsCenterTipAndOppositeOuterShoulders`、`HorizontalEndpointsSupportUpAndDownCenterTips` 等方向断言；
- 字节级反编译扫描 5 套部署 DLL：2007/2010/2013/2015/2025 均含新版几何常量（`0.42`、`0.12`、贝塞尔 `kappa`），且不含旧版 W 形常量（`0.18`/`0.92`/`0.48341`）——部署 DLL 与当前源码一致；
- 根因判定：安装脚本以注册表 `LOADER` 直接指向部署目录内 `PatentMarker.dll`。若同事 CAD 未重新运行 `install-2007.vbs` 覆盖安装（或目标 DLL 未覆盖、AutoCAD 未完全重启），实际加载的是 8-12 修复前旧 DLL。**同事侧处理：重新运行 `install-2007.vbs` → 完全退出并重启 AutoCAD → 复查 PATBRACE 方向**。
- 顺带清理：删除仓库内陈旧目录 `PatentMarker-2007-v2/`（DLL 为 2026-08-04 旧构建、无大括号功能，VBA 未同步，自 1.0.0 起由 `PatentMarker-2007-deploy` 取代），其内容由 `.gitignore` 排除、从未入库。
