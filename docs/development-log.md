...
- Leader/MText 四象限文字附着修正（2026-08-11）：在已有左右侧判断上增加上下方向判断，按引线首点相对文字的位置选择 `TopLeft`、`BottomLeft`、`TopRight` 或 `BottomRight`；引线首点与文字同高时保留 `MiddleLeft`/`MiddleRight`。五个版本继续共用同一附着规则，并补充左上、左下、右上、右下运行契约测试。
