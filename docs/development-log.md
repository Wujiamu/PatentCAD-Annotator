...
## 1.0.3（待发布，2026-09-13）

**Word 自动导出启动链路与安装边界重构（2026-09-12）。** 用户再次确认“手动导出有效、自动保存无效”，并指出旧安装会删除 Word 中已有宏。复核后撤回此前由源码直导入回归得出的“安装后自动导出已通过”：`verify-vba-export.vbs` 会把源码导入临时 docm 并用 `ExportHarness` 主动建立保存钩子，完全绕过了部署包、Word Startup、自动初始化和重启持久化，证据最多属于 L2。旧 `install-vba.vbs` 又直接编辑/保存 `Normal.dotm`，失败恢复包含另存临时模板后覆盖整个 Normal 的路径；它没有非项目组件清单断言，不能反驳用户的宏丢失报告。

- **新安装架构**：根 `vba/` 增加 `PatentMarkerBootstrap.bas`，通过 `AutoExec` 幂等初始化模块级 `clsSaveHook`、`AutoExit` 释放。`tools/build-vba-addin.vbs` 从 8 个组件/9 个源文件构建规范 `word-addin/PatentMarker.dotm`，执行编译探针，并在生成前后逐字节确认 Normal 未变化。五套部署包改为携带同一 dotm；规范 `install-vba.vbs` 只把它暂存、二进制比较并可恢复地移动到 `Application.StartupPath`，不打开、不编辑、不保存 Normal，也不要求 AccessVBOM。`uninstall-vba.vbs` 默认要求所有权清单，只把精确产品文件移动到可恢复备份。
- **可诊断性**：`AutoExport.bas` 为每次 Word 运行建立 run ID，把产品版本、Word 版本/位数、加载模板路径、初始化入口、钩子生命周期、`DocumentBeforeSave` 的文档/路径/SaveAsUI/Cancel、目标字典路径及错误号/来源/描述写入 `%LOCALAPPDATA%\PatentMarker\Logs\word-vba-YYYYMMDD.tsv`；面板状态行读取当前钩子/最近导出状态。安装后回归在进程外记录实际加载文件 SHA-256，避免由待测 VBA 自报哈希。
- **验证入口修正**：新增 `verify-vba-installer.ps1`（隔离 Startup）和 `verify-vba-installed-e2e.ps1` + `verify-vba-installed-runtime.vbs`（真实 Startup）。首次 L4 草案用 `CreateObject` 启动 Word 时模板已加载但钩子为 `DISABLED`，证明自动化创建进程不能作为 AutoExec 证据；最终脚本改为真实 `WINWORD.EXE /q /n` 正常启动后再附加 COM，且不调用初始化器、`ExportDict` 或面板手动导出。`PatentMarkerBootstrap` 带 `Option Private Module` 时同一正常启动用例仍稳定为红；只移除该选项、重新生成相同候选包后转绿，因此该行不得在缺少等价 L4 的情况下加回。
- **本机红绿证据**：L2 修改过程中曾出现 `FAIL|automatic export dictionary missing`，日志定位为测试 docm 被 `ThisDocument` 判断误当模板；移除该错误跳过条件后，同一命令转为 `PASS|L2_SOURCE_IMPORT`。隔离安装回归覆盖首次/重复安装、锁定所有权清单的安装与卸载故障注入、逐字节回滚、可恢复卸载及 Normal/用户宏哨兵零变化，结果为 PASS。最终本机 Word 16.0 64 位 L4 的 run ID 为 `20260912-190823-BEE4C2`：日志依次具有 `origin=AutoExec`、`save.before.enter`、`export.success`、`save.before.exit export_ok=true`；普通保存生成 UTF-8 JSON 并解析出 `1=底座`、`2=支架`。部署、实际 Startup 加载项 SHA-256 均为 `BDCDA016E49409A573EE54527455068E624046EA1433592ED75D47E2CA6D8B4C`，Normal 前后 SHA-256 均为 `96DBA95BF4ECC3D955BA608F3077B0D3D68AE8FC6E1425C86E14B56EBD540D72`，非产品 Startup 文件清单/哈希未变化。
- **2026-09-13 复验与探针竞态修复**：当前安装器进一步移除了仅用于日志的 `NormalTemplate.FullName` 读取，静态门禁禁止安装/卸载器出现 `NormalTemplate`、`VBProject`、`VBComponents` 或 `OrganizerDelete`。首次复验在刚附加 COM 后立即枚举 `Templates`，出现 `FAIL|fresh Word did not load the expected Startup add-in`；随后确认安装已逐字节成功，根因是探针把“COM 可附加”误当成“Startup 加载完成”。探针改为在有限超时内轮询预期模板完整路径和 ENABLED 钩子、失败时输出已加载模板集合。相同候选包最终以 run ID `20260913-102840-3542C3` 转绿：Word 16.0 64 位，加载项 SHA-256 `BDCDA016E49409A573EE54527455068E624046EA1433592ED75D47E2CA6D8B4C`，Normal SHA-256 `96DBA95BF4ECC3D955BA608F3077B0D3D68AE8FC6E1425C86E14B56EBD540D72` 前后不变，普通保存自动导出通过；脱敏产物保留在 `%TEMP%\PatentMarker-Installed-E2E-5a241fcac43b4dd4b3e8b90107b05524`，日志为 `%LOCALAPPDATA%\PatentMarker\Logs\word-vba-20260913.tsv`。
- **面板源码宿主复验**：`tools/test-vba-panel.vbs` 不再把带参数的诊断过程误算成 Alt+F8 入口，而是解析并断言无参公开过程的确切名称。真实 Word 临时文档导入 8 个组件/9 个物理文件后，结果为 `PASS|L2_PANEL_SOURCE_IMPORT`：公开无参入口仅 `ShowPatentDictPanel`、`AutoExec`、`AutoExit`，UserForm 探针通过并生成可解析 JSON。该结果仍只属于 L2，不替代安装后的宏列表截图或 L4 Startup 证据。
- **扩展矩阵先红后绿——属性恢复**：新增安装后黑盒矩阵第一次锁定已有字典时稳定得到 `FAIL|locked export changed dictionary attributes from 6 to 0`。根因是覆盖前清除了 Hidden/System，替换失败时没有恢复。`AutoExport.bas` 现在记录完整原属性，临时只清除必要位，并在成功/失败两条路径恢复；锁定和只读故障都会断言旧 JSON 字节、全部属性、文档未保存状态、错误文件与故障解除后的恢复保存。
- **扩展矩阵先红后绿——DOTM 宏发现元数据**：修复属性后重新生成的 dotm 虽有 `word/vbaProject.bin` 且能作为 Startup 模板加载，但钩子仍为 `DISABLED`。与此前绿包逐项比较确认新包缺少 `word/vbaData.xml` 及 `word/_rels/vbaProject.bin.rels`；仅让 Word 再保存一次也未生成它们。新增 `finalize-dotm-package.ps1` 确定性写入关系和 3 个宏的元数据，`verify-dotm-package.ps1` 在构建、Static 和打包阶段要求 `ShowPatentDictPanel`、`AutoExec`、`AutoExit` 恰好存在。修复后正常 WINWORD 再次由 AutoExec 自动挂钩。
- **扩展矩阵先红后绿——临时文件清理**：下一轮锁定目标稳定得到 `FAIL|locked export left a temporary dictionary file`，日志为 ADODB stream 已关闭时再次 Close 的 3704 次生错误。`JsonWriter.bas` 现在分别检查两个 stream 的 State、清除次生错误后删除本次唯一临时文件；同一锁定场景随后通过且临时文件计数为 0。
- **扩展矩阵先红后绿——退出诊断会话漂移**：继续核对最终日志时发现，Word 退出阶段若宿主已清空 VBA 模块状态，`AutoExit` 会重新初始化诊断状态并生成一个只含 `hook.release` 的伪 run ID。新增退出前后日志差异断言后，旧候选稳定得到 `FAIL|Word shutdown created a foreign diagnostic run ID`；`ReleaseAutoExportForShutdown` 现在在没有既有诊断会话和活动钩子时直接返回，类终止跟踪也不再创建会话。相同 L4 命令随后转绿。
- **最终本机 L4 扩展证据**：候选 dotm、五套部署副本和实际 Startup 文件 SHA-256 均为 `2F3E4C46DAAC61225648EA5905507EAAC6EBF602947F074C93CE2CEF91457BC8`。Word 16.0 64 位先以 run ID `20260913-210320-74DFC3` 完成基础正常启动/普通保存，再以独立 run ID `20260913-210340-4EBEC3` 完成多文档隔离、Save As 改名/换目录及后续保存、锁定/只读目标的取消保存与恢复、Word 运行中拒绝安装及卸载；两次运行均通过退出阶段无外来 run ID 的断言。Normal 前后 SHA-256 都为 `96DBA95BF4ECC3D955BA608F3077B0D3D68AE8FC6E1425C86E14B56EBD540D72`，非产品 Startup 快照不变。脱敏产物保留在 `%TEMP%\PatentMarker-Installed-E2E-cffaa582c9be40a5ae225a5d75248037`。Save As 在同一次操作后的新路径没有立即 JSON（`save_as_immediate_export=false`），随后一次普通保存通过，因此仍按真实边界提示用户。
- **最终回归与发行暂存**：L2 源码导出和面板导入均 PASS；隔离安装器覆盖缺失所有权清单拒绝卸载、首次/重复安装、安装/卸载故障回滚及非产品哨兵零变化；Structure、Static、VBA/Word/MLeader/共享源同步、DOTM 包检查和 SDK API 面均 PASS。DOTM 验证器会同时断言工程、内容类型、关系及 3 个宏元数据，故障注入删除关系文件后按预期失败且未改写样本。五版生产项目全部编译，2025 单测 `120/120`，2007/2010/2013/2015 模拟宿主各 `33/33`；最终候选的全版本发行暂存位于 `%TEMP%\PatentCAD-Annotator-release-20260913-210920`，五个包的 DOTM 元数据均通过且 SHA-256 都为 `2F3E4C46DAAC61225648EA5905507EAAC6EBF602947F074C93CE2CEF91457BC8`，禁止的日志/LSP/备份/报告文件计数为 0。完整矩阵和 BLOCKED 项见 `docs/word-vba-acceptance-2026-09-13.md`。
- **2026-09-14 CAD doctor 判定加固**：真实 AutoCAD 2026 暂存验证暴露两类相反误判。注册表 demand-load 先加载部署 DLL 后，SCR 中对暂存 DLL 的 `NETLOAD` 不会替换已加载的同名程序集，PATDOCTOR 实际把报告写到部署目录；旧外部 doctor 只轮询暂存目录，等待 300 秒后误报“未生成”。另一方面，`tools/doctor-live-test.ps1` 过去只要看到报告文件就输出 `TEST DONE` 并返回 0，即使报告正文为 `PASS 3 / FAIL 1 / SKIP 2`。现已要求核对报告中的实际程序集完整路径和可解析 Summary；2025 外部 doctor 遇到 in-CAD FAIL 时返回非零，发现注册的 loader 会抢载另一份候选时快速 WARN/跳过；本机部署路径复测得到显式 `PASS|CAD_HOST_LOAD|...|expected_unconfigured_dictionary=true`，而发布 doctor 对同一未配置字典报告返回 1。暂存包在主机身份下确认 `PASS=4 FAIL=0 WARN=1` 且未启动 AutoCAD。进一步用已保存临时 DWG 做字典红绿回归时发现，DictLoader 解析异常返回 `null` 后旧 PATDOCTOR 会把它误算成 0 条合法字典并 SKIP；共享诊断代码现将该情况明确记为 FAIL。五版重新编译/打包后，损坏 `occurrences` 类型的 JSON 得到 `PASS 3 / FAIL 1 / SKIP 2 | recent errors: 1`，修正后的合法 1 条目 JSON 得到 `PASS 4 / FAIL 0 / SKIP 2 | recent errors: 0`，两次报告均核对实际加载 2025 部署 DLL 路径。
- **仍未覆盖**：Word 2010、32 位 Office、不同宏策略、交互式取消 Save As、VBA Reset 及 ACL/目录权限拒绝仍需目标环境或 UI 回归。本机对 Normal VBProject 做只读组件清点返回错误 6068（策略或项目保护），因此没有自动清理旧版本留在 Normal 内的项目组件；新安装器继续保持 Normal 完全不动。当前 Codex 环境没有原生 Word UI 控制接口，本轮也未对 Alt+F8 宏列表补截图证据。

**字典文件可见性可选化。** CAD 字典面板新增“显示 JSON/隐藏 JSON”按钮，Word 面板新增手动编辑可见性开关；切换只修改 Windows Hidden/System 属性，不改 JSON 内容。Word 与五版 CAD 写回现在保留用户已经选择的可见状态，新建字典仍默认隐藏。共享可见性辅助、五版写回、VBA 根源和五套部署包已同步；2025 单元测试覆盖默认隐藏、可见状态写回保留和属性切换往返。

可见状态只表示用户选择让文件出现在资源管理器中，不把 JSON 文本编辑转换成 CAD 图纸事务。手工修改后应在 CAD 面板重载并核对 JSON；Word/CAD 同时写入仍按现有字典冲突与原子替换规则处理。
...
**Word 工具面板 UI 优化 + 导出状态反馈（`.frm`/`.frx`，5 套部署包 + 2007-v2 同步，SHA256 一致）。**

- **界面放大、字号加大**：窗体扩至 300×178 磅；字体统一为微软雅黑——主按钮 14pt 加粗、复选框 11pt、状态行 10pt；主按钮改用蓝色（RGB 0,120,215）白字，整体更清晰、更简洁、更美观。布局在 `UserForm_Initialize` 内以磅（points）为单位显式设置，独立于 `.frx` 设计数据，从根上避免 twip 量级坐标把控件排出客户区的"小空框"。

- **导出状态反馈**：`cmdExport_Click` 新增底部状态行 `lblStatus`，点击"手动导出字典"后实时反馈——成功置绿色"√ 已导出 HH:MM:SS"，失败置红色"× 导出失败（文档未保存？）"，解决此前点击后无任何视觉提示的问题。

- **`.frx`** **权威重生成**：此前手工编辑 `.frx`（OLE 复合文档）导致面板左上角残留"幽灵文字"残影；通过 `_paneltest/regenerate-frx.ps1` + `builder.bas` 用 Word 设计器重新生成权威 `.frm`/`.frx`，消除了残影。

- **验证（全部本地实际执行）**：Word 实机重放导入 + 运行时属性查询（窗体/按钮/复选框/状态行坐标与字体）+ 截屏像素分析（蓝色按钮带与白色文本行），确认按钮、复选框、状态行正常显示且无残影；`build.ps1 -Static` 全绿（VBA 跨包一致 + Shared 单源层 + 版本本地一致）。

- **兼容**：用户重新运行 `install-vba.vbs`（或手动重导入 VBA）即可获得本版面板；与 1.0.0 生成的隐藏 `.dict.json` 双向同步行为无变化。
...
**发布测试期修复（2026-08-18）**：`PatentDictPanel` 面板显示为空白色小框——`.frx` 设计数据由程序生成，控件坐标误用 twip 量级（MSForms 实际单位为磅/points），控件全部落在客户区外。修复：`UserForm_Initialize` 中以磅为单位显式重设窗体与控件布局；窗体标题改为"专利标注字典工具"。已同步 5 套部署包 + 2007-v2（`.frm` 哈希一致），Word 实机验证通过（运行时属性查询 + 截图像素分析）。用户重新运行 `install-vba.vbs`（或手动重导入 VBA）即可获得修复。
